...
abstract class AnnouncementLocalDao {
  ...
  @Query('SELECT * FROM AnnouncementLocal WHERE announcementID = :announcementID')
  ...
  Future<void> updateRead(String announcementID ,String unread);

  @Query(
      'SELECT * FROM AnnouncementLocal WHERE category = (:category) AND action = (:action)')
  Future<List<AnnouncementLocal>> getAnnouncementsBy(String category, String action);
