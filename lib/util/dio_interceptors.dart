...
class PostcardAuthInterceptor extends Interceptor {
  void addXApiTimestamp(RequestOptions options) {
    final timestamp =
        (DateTime.now().millisecondsSinceEpoch ~/ 1000).toString();
  ...
  //   try {
  //     final errorBody = err.response?.data as Map<String, dynamic>;
  //     err.error = PostcardError.fromJson(errorBody["error"]);
  //   } catch (e) {
  //     log.info(
  //         "[PostcardAuthInterceptor] Can't parse error. ${err.response?.data}");
  //   } finally {
  //     handler.next(err);
  //   }
  // }
