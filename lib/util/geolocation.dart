  ...
  if (!hasPermission) {
    ...
    try {
      ...
      if (location.isMocked) {
        unawaited(UIHelper.showMockedLocation(
      ...
      log.info('Fuzzy Location: ${geolocation.position.lat}, '
...
bool isValidLocation(Location position, double latitude, double longitude) =>
    (position.latitude - latitude).abs() < 0.2 &&
...
class GeoLocation {
  ...
  bool get isInternet => this == internetUserGeoLocation;
...
extension PositionExtension on Position {
  ...
  postcard.Location toLocation() =>
      postcard.Location(lat: latitude, lon: longitude);
}
